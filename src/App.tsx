...
function XHSLoginModal({ onClose }: { onClose: () => void }) {
    ...
    const check = async () => {
        try {
            const r = await fetch('http://localhost:3003/api/status', { mode: 'cors' });
        ...
        } catch { setStep('error'); setMsg('无法连接 API。请确认终端 3003 端口服务正在运行'); if (pollRef.current) clearInterval(pollRef.current); }
    };

    const startLogin = async () => {
        setStep('wait'); setMsg('');
        try {
            await fetch('http://localhost:3003/api/action', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'login' }), mode: 'cors' });
        } catch { }
                    ...
                    <div style={{ textAlign: 'center' }}>
                        {qrReady ? (
                            <>
                                ...
                                <img key={imgKey} src={`http://localhost:3003/api/qr?t=${imgKey}`} alt="登录二维码" style={{ width: 200, height: 200, borderRadius: 10, border: '1px solid #eee' }} />
...
// 豆包登录/状态弹窗
function DoubaoLoginModal({ onClose }: { onClose: () => void }) {
    const [step, setStep] = useState<'info' | 'wait' | 'done' | 'error'>('info');
    const [msg, setMsg] = useState('');
    const [qrReady, setQrReady] = useState(false);
    const pollRef = useRef<ReturnType<typeof setInterval> | null>(null);
    ...
    const check = async () => {
        try {
            const r = await fetch('http://localhost:3004/api/status', { mode: 'cors' });
            const d = await r.json();
            if (d.loggedIn) { setStep('done'); setMsg(`已登录！有效期剩余 ${d.expiresIn || '7 days'}`); if (pollRef.current) clearInterval(pollRef.current); setTimeout(onClose, 2500); return; }

            if (d.qrExists) { setQrReady(true); setImgKey(k => k + 1); }
        } catch { setStep('error'); setMsg('无法连接 API。请确认终端 3004 端口服务正在运行'); if (pollRef.current) clearInterval(pollRef.current); }
    };

    const startLogin = async () => {
        setStep('wait'); setMsg('');
        try {
            await fetch('http://localhost:3004/api/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}', mode: 'cors' });
        } catch { }
        pollRef.current = setInterval(check, 4000);
    };
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.72)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ background: '#fff', borderRadius: 20, padding: 32, maxWidth: 440, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.4)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                    <h2 style={{ margin: 0, fontSize: 18, fontWeight: 900 }}>🔑 豆包自动化生图</h2>
                    <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#aaa' }}>×</button>
                </div>
                {step === 'info' && (
                    <div>
                        <p style={{ color: '#555', lineHeight: 1.9, fontSize: '14px', marginBottom: 12 }}>
                            本功能将拉起本地浏览器自动访问豆包网页版，并截取AI生成的图片。（登录只需1次，有效期7天）。
                        </p>
                        <div style={{ display: 'flex', gap: 10 }}>
                            <button onClick={startLogin} style={{ flex: 1, padding: '12px', background: '#e11d48', color: '#fff', border: 'none', borderRadius: 10, cursor: 'pointer', fontWeight: 800, fontSize: 14 }}>🔑 扫码登录豆包</button>
                            <button onClick={onClose} style={{ padding: '12px 18px', background: '#f5f5f5', border: 'none', borderRadius: 10, cursor: 'pointer', fontWeight: 700 }}>暂不</button>
                        </div>
                    </div>
                )}
                {step === 'wait' && (
                    <div style={{ textAlign: 'center' }}>
                        {qrReady ? (
                            <>
                                <p style={{ fontSize: '13px', color: '#666', marginBottom: 10 }}>用豆包App扫描二维码登录</p>
                                <img key={imgKey} src={`http://localhost:3004/api/qr?t=${imgKey}`} alt="登录二维码" style={{ width: 200, height: 200, borderRadius: 10, border: '1px solid #eee' }} />
                                <p style={{ fontSize: '12px', color: '#059669', marginTop: 10 }}>⏳ 等待扫码中…</p>
                            </>
                        ) : (
                            <div style={{ padding: '30px 0' }}><div style={{ fontSize: 36, marginBottom: 10 }}>⏳</div><p style={{ color: '#666' }}>正在启动浏览器，请稍等15秒…</p></div>
                        )}
                ...
                {step === 'done' && (
                    <div style={{ textAlign: 'center', padding: '20px 0' }}>
                        <div style={{ fontSize: 48, marginBottom: 12 }}>🎉</div>
                        <p style={{ fontSize: 16, fontWeight: 800, color: '#059669' }}>{msg || '登录成功！'}</p>
                        <p style={{ fontSize: 13, color: '#888', marginTop: 8 }}>现在可以「用豆包生成图片」了！</p>
                    </div>
                )}
                {step === 'error' && (
                    <div style={{ textAlign: 'center', padding: '10px 0' }}>
...
export default function App() {
    ...
    const checkBothLogins = async () => {
        try {
            const r = await fetch('http://localhost:3003/api/status', { mode: 'cors' });
            const d = await r.json();
            setXhsStatus(prev => ({ ...prev, api: 'connected', login: d.loggedIn ? 'logged_in' : 'not_logged_in', sessionAge: d.expiresIn }));
        } catch { setXhsStatus(prev => ({ ...prev, api: 'disconnected', login: 'checking' })); }

        try {
            const r2 = await fetch('http://localhost:3004/api/status', { mode: 'cors' });
            const d2 = await r2.json();
            setXhsStatus(prev => ({ ...prev, doubaoLoggedIn: d2.loggedIn }));
        } catch { setXhsStatus(prev => ({ ...prev, doubaoLoggedIn: false })); }
    };

    useEffect(() => {
        checkBothLogins();
        const id = setInterval(checkBothLogins, 20000);
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            if (xhsStatus.api === 'connected' && xhsStatus.login === 'logged_in') {
                ...
                const resp = await fetch('http://localhost:3003/api/publish', {
    ...
    const handleDoubaoGenerate = async () => {
        ...
        setDoubaoResult('');
        try {
            const resp = await fetch('http://localhost:3004/api/generate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (d.ok) {
                ...
                for (let i = 0; i < 60; i++) {
                    ...
                    try {
                        const r2 = await fetch('http://localhost:3004/api/result', { mode: 'cors' });
                        const d2 = await r2.json();
    ...
    if (review) {
        ...
        return (
            <div style={{ minHeight: '100vh', background: '#f5f0e8', fontFamily: '"PingFang SC","Microsoft YaHei",sans-serif' }}>
                ...
                {doubaoLoginOpen && <DoubaoLoginModal onClose={() => { setDoubaoLoginOpen(false); checkBothLogins(); }} />}
