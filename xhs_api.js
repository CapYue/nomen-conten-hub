...
const LOGIN_SCRIPT = '/workspace/pw/xhs_login.js';
const PUBLISH_SCRIPT = '/workspace/pw/xhs_publish.js';
...
// 后台运行登录（spawn，不阻塞）
function startLogin() {
  const child = spawn('xvfb-run', ['-a', 'node', LOGIN_SCRIPT], { detached: true, stdio: 'ignore' });
  child.unref();
...
function runPublish({ title, content, images, topics }) {
  ...
  try {
    const result = execSync(`xvfb-run -a node ${PUBLISH_SCRIPT} "${title.replace(/"/g, '\\"')}" "${content.replace(/"/g, '\\"').substring(0, 200)}" "${(topics || '').replace(/"/g, '\\"')}"`, {
...
function handleRequest(req, res) {
  ...
  if (req.method === 'GET' && req.url === '/api/status') {
    ...
    res.end(JSON.stringify({ ok: true, version: '2.1', ...login, chromePath: fs.existsSync(CHROME) ? 'found' : 'missing', qrExists: fs.existsSync('/workspace/pw/xhs_login_qr.png'), lastCheck: new Date().toISOString() }));
    return;
  }

  // GET /api/qr
  if (req.method === 'GET' && req.url.startsWith('/api/qr')) {
    const qrFile = '/workspace/pw/xhs_login_qr.png';
    ...
    req.on('end', () => {
      ...
      try {
        ...
        if (action === 'status' || action === 'ping') {
          const login = checkLogin();
          res.writeHead(200, { ...cors, 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true, ...login, qrExists: fs.existsSync('/workspace/pw/xhs_login_qr.png') }));
          return;
        }
  ...
  if (req.method === 'POST' && req.url === '/api/publish') {
    ...
    req.on('end', () => {
      ...
      try {
        ...
        // 后台执行，不阻塞
        spawn('xvfb-run', ['-a', 'node', PUBLISH_SCRIPT, title, content + '\n\n' + (topics || '')], {
          detached: true, stdio: 'ignore', cwd: '/workspace/pw'
        }).unref();
...
if (fs.existsSync('/workspace/pw/cert.pem') && fs.existsSync('/workspace/pw/key.pem')) {
  https.createServer({ cert: fs.readFileSync('/workspace/pw/cert.pem'), key: fs.readFileSync('/workspace/pw/key.pem') }, handleRequest)
